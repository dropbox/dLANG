...
├─────────────────────────────────────────────────────────────┤
│  trust_macros (proc_macro)  →  VC IR  →  Z4               │
...
│       ↓                                                     │
│  rustc_verify    → NEW PASS: MIR → VC → Z4 → prove/disprove │
│       ↓                                                     │
...
└── rustc_verify/       ← NEW CRATE: verification pass
    ...
    │   └── solver.rs   ← Z4/Z3 integration
    └── Cargo.toml
```

## The Plan

1. **NOW**: Use proc_macros to test spec syntax, build VC IR, integrate Z4
...
2. Test VC → Z4 pipeline without touching rustc
