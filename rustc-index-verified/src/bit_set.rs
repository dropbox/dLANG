...
impl<T: Idx> DenseBitSet<T> {
    ...
    /// Clear all elements.
    #[inline]
    #[cfg_attr(trust_verify, ensures("self.domain_size() == old(self.domain_size())"))]
    ...
    #[inline]
    #[cfg_attr(trust_verify, pure)]
    #[cfg_attr(trust_verify, requires("elem.index() < self.domain_size"))]
    ...
    #[inline]
    #[cfg_attr(trust_verify, pure)]
    #[cfg_attr(trust_verify, requires("self.domain_size() == other.domain_size()"))]
    ...
    #[inline]
    #[cfg_attr(trust_verify, requires("elem.index() < self.domain_size"))]
    #[cfg_attr(trust_verify, ensures("self.contains(elem)"))]
    #[cfg_attr(trust_verify, ensures("self.domain_size() == old(self.domain_size())"))]
...
impl<T: Idx> ChunkedBitSet<T> {
    ...
    /// Creates a new, empty bitset with a given `domain_size`.
    // SOLVER_LIMIT: L13 - postconditions through private method delegation not provable
    // (applies to both is_empty() and domain_size())
    #[inline]
            ...
            self.chunks.iter_mut().zip(other.chunks.iter()).enumerate()
        {
            ...
            match (&mut self_chunk, &other_chunk) {
                ...
                (Ones, Mixed(other_chunk_count, other_chunk_words)) => {
                    ...
                    let last_word_bits =
                        (chunk_domain_size as usize) - ((num_words - 1) * WORD_BITS);
                    debug_assert!((1..=WORD_BITS).contains(&last_word_bits));
                    let mut tail_mask = if last_word_bits == WORD_BITS {
                        Word::MAX
                    } else {
...
impl<T: Idx> GrowableBitSet<T> {
    ...
    #[cfg_attr(trust_verify, ensures("self.bit_set.domain_size >= min_domain_size"))]
    #[cfg_attr(
        trust_verify,
        ensures("self.bit_set.domain_size >= old(self.bit_set.domain_size)")
    )]
    ...
    /// Returns `true` if the set has changed.
    ///
    /// # Solver Limitation (L13)
    ///
    /// Cannot verify postcondition `self.contains(elem)` because:
    /// - `ensure()` establishes `self.bit_set.domain_size >= elem.index() + 1`
    /// - `DenseBitSet::insert` requires `elem.index() < self.domain_size`
    /// - The solver cannot flow postconditions through struct field paths
    ///
    /// See `tests/solver_limitations.rs` L13 for a minimal reproduction.
    #[inline]
    pub fn insert(&mut self, elem: T) -> bool {
    ...
    /// Cannot verify postcondition `!self.contains(elem)` because:
    /// - `ensure()` establishes `self.bit_set.domain_size >= elem.index() + 1`
    /// - `DenseBitSet::remove` requires `elem.index() < self.domain_size`
    /// - The solver cannot flow postconditions through struct field paths
    ///
    /// See `tests/solver_limitations.rs` L13 for a minimal reproduction.
    #[inline]
    ...
    #[cfg_attr(trust_verify, pure)]
    pub fn contains(&self, elem: T) -> bool {
        if elem.index() < self.bit_set.domain_size {
            self.bit_set.contains(elem)
        } else {
            false
        }
    }
...
impl<R: Idx, C: Idx> BitMatrix<R, C> {
    ...
    #[cfg_attr(trust_verify, ensures("result.num_rows == num_rows"))]
    #[cfg_attr(trust_verify, ensures("result.num_columns == num_columns"))]
    ...
    #[cfg_attr(trust_verify, ensures("result.num_rows == num_rows"))]
    #[cfg_attr(trust_verify, ensures("result.num_columns == row.domain_size()"))]
    ...
    #[cfg_attr(
        trust_verify,
        requires("row.index() < self.num_rows && column.index() < self.num_columns")
    )]
    #[cfg_attr(trust_verify, ensures("self.contains(row, column)"))]
    #[cfg_attr(trust_verify, ensures("self.num_rows == old(self.num_rows)"))]
    #[cfg_attr(trust_verify, ensures("self.num_columns == old(self.num_columns)"))]
    ...
    #[cfg_attr(
        trust_verify,
        requires("row1.index() < self.num_rows && row2.index() < self.num_rows")
    )]
    ...
    #[cfg_attr(
        trust_verify,
        requires("read.index() < self.num_rows && write.index() < self.num_rows")
    )]
    #[cfg_attr(trust_verify, ensures("self.num_rows == old(self.num_rows)"))]
    #[cfg_attr(trust_verify, ensures("self.num_columns == old(self.num_columns)"))]
    ...
    #[cfg_attr(
        trust_verify,
        requires("write.index() < self.num_rows && with.domain_size() == self.num_columns")
    )]
    #[cfg_attr(trust_verify, ensures("self.num_rows == old(self.num_rows)"))]
    #[cfg_attr(trust_verify, ensures("self.num_columns == old(self.num_columns)"))]
    ...
    #[cfg_attr(trust_verify, requires("row.index() < self.num_rows"))]
    #[cfg_attr(trust_verify, ensures("self.num_rows == old(self.num_rows)"))]
    #[cfg_attr(trust_verify, ensures("self.num_columns == old(self.num_columns)"))]
...
impl<R: Idx, C: Idx> SparseBitMatrix<R, C> {
    ...
    ///
    /// Returns `true` if this changed the matrix.
    // SOLVER_LIMIT: L13 - postcondition `self.contains(row, column)` unprovable through ensure_row delegation
    #[cfg_attr(trust_verify, requires("column.index() < self.num_columns"))]
    #[cfg_attr(trust_verify, ensures("self.num_columns == old(self.num_columns)"))]
    pub fn insert(&mut self, row: R, column: C) -> bool {
        assert!(column.index() < self.num_columns);
        let row = self.ensure_row(row);
        assert!(column.index() < row.domain_size());
        row.insert(column)
    }
...
impl<T: FiniteBitSetTy> FiniteBitSet<T> {
    ...
    /// Returns whether the index is within the domain of the bitset.
    #[cfg_attr(trust_verify, pure)]
