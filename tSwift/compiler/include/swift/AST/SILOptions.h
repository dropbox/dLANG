...
public:
  ...
  /// Enable formal verification of @_requires/@_ensures specifications.
  bool EnableFormalVerification = false;

  /// Treat formal verification failures as errors instead of warnings.
  bool VerifyStrict = false;

  /// SMT solver timeout for formal verification in milliseconds.
  unsigned VerifyTimeout = 5000;

  /// Path to output JSON verification report. Empty means no report.
  std::string VerifyReportPath;

  /// Path to output JSON for vc-ir-swift bridge. Empty means no export.
  /// When set, emits SwiftVcBundle JSON for each function with specs/autoVCs.
  std::string VerifyJsonExportPath;

  /// Verify all functions, not just those with @_requires/@_ensures specs.
  bool VerifyAllFunctions = false;
