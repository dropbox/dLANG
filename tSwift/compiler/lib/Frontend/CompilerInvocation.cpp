...
static bool ParseSILArgs(SILOptions &Opts, ArgList &Args,
  ...
  Opts.EnableFormalVerification |= Args.hasArg(OPT_enable_formal_verification);
  Opts.VerifyStrict |= Args.hasArg(OPT_verify_strict);
  if (const Arg *A = Args.getLastArg(OPT_verify_timeout)) {
    if (StringRef(A->getValue()).getAsInteger(10, Opts.VerifyTimeout)) {
      Diags.diagnose(SourceLoc(), diag::error_invalid_arg_value,
                     A->getAsString(Args), A->getValue());
    }
  }
  Opts.VerifyReportPath = Args.getLastArgValue(OPT_verify_report).str();
  Opts.VerifyJsonExportPath = Args.getLastArgValue(OPT_verify_json_export).str();
  Opts.VerifyAllFunctions |= Args.hasArg(OPT_verify_all_functions);
